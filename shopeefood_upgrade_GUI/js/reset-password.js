...
function showInitialInstructions() {
    setTimeout(() => {
        showPopupMessage('info', 'Password Requirements', 
            'Your new password must be at least 8 characters long and include uppercase letters, lowercase letters, numbers, and special characters for maximum security.', {
            autoClose: true,
            autoCloseDelay: 6000
        });
    }, 1000);
...
function goBackToLogin() {
    showPopupMessage('info', 'Returning to Login', 'Redirecting to login page...', {
        autoClose: true,
        autoCloseDelay: 1500,
        onClose: () => {
            window.location.href = '/login';
...
// Password history check (simulated)
function checkPasswordHistory(password) {
    // In real implementation, this would check against user's password history
    ...
    return !simulatedHistory.includes(password);
}
...
function advancedPasswordValidation(password) {
    ...
    if (/(.{2,})\1+/.test(password)) {
        issues.push('Avoid repeated patterns');
    }
    
    // Check for common substitutions
    const commonSubs = password
        .replace(/[@4]/g, 'a')
        .replace(/[3]/g, 'e')
        .replace(/[1!]/g, 'i')
        .replace(/[0]/g, 'o')
    ...
    if (weakPasswords.includes(commonSubs.toLowerCase())) {
        issues.push('Avoid common letter-to-number substitutions');
    }
...
function animateStrengthMeter(targetWidth) {
    ...
    const animation = setInterval(() => {
        ...
        if (currentStep >= 20 || Math.abs(newWidth - targetWidth) < 1) {
            strengthFill.style.width = `${targetWidth}%`;
            clearInterval(animation);
        } else {
            strengthFill.style.width = `${newWidth}%`;
        }
    }, 20);
}

// Enhanced password strength calculation
function calculateAdvancedPasswordStrength(password) {
    let score = 0;
    let feedback = [];
    let bonusPoints = 0;
    ...
    else feedback.push(`Add ${8 - password.length} more characters`);
    
    if (password.length >= 12) bonusPoints += 1;
    if (password.length >= 16) bonusPoints += 1;
    if (password.length >= 20) bonusPoints += 2;
    ...
    if (uniqueChars >= password.length * 0.7) bonusPoints += 1;
    
    // Entropy calculation
    const entropy = Math.log2(Math.pow(95, password.length));
    if (entropy >= 60) bonusPoints += 1;
    if (entropy >= 80) bonusPoints += 1;
    
    // Penalize common patterns
    const advancedIssues = advancedPasswordValidation(password);
    score -= advancedIssues.length;
    feedback.push(...advancedIssues);
    
    const finalScore = Math.max(0, Math.min(10, score + bonusPoints));
...
// Copy password to clipboard (for generated passwords)
async function copyToClipboard(text) {
    try {
        await navigator.clipboard.writeText(text);
        showPopupMessage('success', 'Copied!', 'Password copied to clipboard.', {
            autoClose: true,
            autoCloseDelay: 2000
        });
    } catch (err) {
...
function animatePasswordReveal(input) {
    ...
    overlay.style.background = 'rgba(102, 126, 234, 0.1)';
    overlay.style.borderRadius = '12px';
    overlay.style.opacity = '0';
    overlay.style.transition = 'opacity 0.3s ease';
    overlay.style.pointerEvents = 'none';
    
    input.closest('.input-wrapper').appendChild(overlay);
...
function enhancedTogglePasswordVisibility(input, toggle) {
    animatePasswordReveal(input);
    setTimeout(() => {
        togglePasswordVisibility(input, toggle);
    }, 100);
}

// Update toggle event listeners with enhanced version
newPasswordToggle.removeEventListener('click', togglePasswordVisibility);
confirmPasswordToggle.removeEventListener('click', togglePasswordVisibility);

newPasswordToggle.addEventListener('click', () => enhancedTogglePasswordVisibility(newPasswordInput, newPasswordToggle));
confirmPasswordToggle.addEventListener('click', () => enhancedTogglePasswordVisibility(confirmPasswordInput, confirmPasswordToggle));

// Auto-save form state (optional)
function saveFormState() {
    const formState = {
        timestamp: Date.now(),
        hasContent: newPasswordInput.value.length > 0 || confirmPasswordInput.value.length > 0
    };
    
    // Don't save actual passwords for security
    sessionStorage.setItem('resetPasswordFormState', JSON.stringify(formState));
}

// Load form state
function loadFormState() {
    try {
        const savedState = JSON.parse(sessionStorage.getItem('resetPasswordFormState'));
        if (savedState && savedState.hasContent) {
            // Show a message that the form had content
            showPopupMessage('info', 'Form Restored', 'Your previous form session has been restored.', {
                autoClose: true,
                autoCloseDelay: 3000
            });
        }
    } catch (error) {
        console.log('No previous form state found');
    }
}

// Auto-save on input (debounced)
const debouncedSaveState = debounce(saveFormState, 1000);
newPasswordInput.addEventListener('input', debouncedSaveState);
confirmPasswordInput.addEventListener('input', debouncedSaveState);

// Clear form state on successful submission
function clearFormState() {
    sessionStorage.removeItem('resetPasswordFormState');
}

// Add to successful submission
const originalHandlePasswordReset = handlePasswordReset;
handlePasswordReset = async function(event) {
    const result = await originalHandlePasswordReset(event);
    if (result !== false) {
        clearFormState();
    }
    return result;
...
function getValidationSummary() {
    const summary = {
        newPassword: {
            valid: passwordValidationState.isNewPasswordValid,
            strength: passwordValidationState.strengthScore,
            issues: []
        },
        confirmPassword: {
            valid: passwordValidationState.isConfirmPasswordValid,
            matches: passwordValidationState.passwordsMatch
        },
        overall: {
            canSubmit: passwordValidationState.isNewPasswordValid && 
                      passwordValidationState.isConfirmPasswordValid && 
                      passwordValidationState.passwordsMatch &&
                      passwordValidationState.strengthScore >= 4
        }
    };
...
// Enhanced validation with announcements
function validateWithAnnouncement(field, isValid, message) {
    if (isValid) {
        announceToScreenReader(`${field} is valid`);
    } else {
        announceToScreenReader(`${field} error: ${message}`);
    }
}
...
const performanceMetrics = {
    startTime: Date.now(),
    validationCalls: 0,
    strengthCalculations: 0
};

// Track performance
function trackPerformance(metric) {
    performanceMetrics[metric] = (performanceMetrics[metric] || 0) + 1;
}

// Add performance tracking to key functions
const originalValidateNewPassword = validateNewPassword;
validateNewPassword = function() {
    trackPerformance('validationCalls');
    return originalValidateNewPassword.apply(this, arguments);
};

const originalCalculatePasswordStrength = calculatePasswordStrength;
calculatePasswordStrength = function() {
    trackPerformance('strengthCalculations');
    return originalCalculatePasswordStrength.apply(this, arguments);
};
...
//     window.resetPasswordDev = {
//         fillTestData: () => {
//             newPasswordInput.value = 'TestPassword123!';
//             confirmPasswordInput.value = 'TestPassword123!';
//             handleNewPasswordInput();
//             handleConfirmPasswordInput();
//         },
//         getMetrics: () => performanceMetrics,
//         getValidationState: () => passwordValidationState,
//         generatePassword: generateStrongPassword
//     };
    
//     console.log('Reset Password Form initialized with development tools');
//     console.log('Use window.resetPasswordDev for testing utilities');
// }
